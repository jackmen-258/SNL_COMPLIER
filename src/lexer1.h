...
void GetTokenList()
{
    string str1="D:\\Code\\Curriculum design\\Compilers\\",str2;
    cin>>str2;
...
void PrintTokenList()
{
    ...
    FILE *file=fopen("D:\\Code\\Curriculum design\\Compilers\\lex_result.txt","w");
    fprintf(file,"%s","\n       对源代码进行词法分析，得到TOKEN信息如下：\n\n\n");
    fprintf(file,"%20s %30s %40s","行号","词法信息","语义信息\n\n\n");
    vector<TokenNode*>::iterator it;
    for(it=TokenList.begin(); it!=TokenList.end();it++)
    {
        fprintf(file,"%20d %30s %40s",(*it)->LineNum-32765,(*it)->LexStr.c_str(),(*it)->SemStr.c_str());
        fprintf(file,"%s","\n");
    }
